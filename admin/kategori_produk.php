<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($koneksi,"delete from kategori_produk WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"insert into kategori_produk Values(NULL,'$nama','$deskripsi')");
			if($q){ alert("Success"); redir("kategori_produk.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"update kategori_produk SET nama='$nama',deskripsi='$deskripsi' where id=$_GET[id]") or die(mysqli_error());
			if($q){ alert("Success"); redir("kategori_produk.php"); }
		}
	}
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($koneksi,"select*from kategori_produk where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
