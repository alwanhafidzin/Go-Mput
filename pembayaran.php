 <?php
  ...
  if(!empty($_GET)){
    if($_GET['act'] == 'delete'){
      ...
      $q = mysqli_query($koneksi,"delete from pesanan WHERE id='$_GET[id]'");
	...
	$user = mysqli_fetch_object(mysqli_query($koneksi,"SELECT*FROM user where id='$_SESSION[iam_user]'"));

	include"layout/header.php";

	$q = mysqli_query($koneksi,"select*from pesanan where user_id='$_SESSION[iam_user]' And status='belum lunas'");
	$j = mysqli_num_rows($q);
?>
...
<?php
	if(!empty($_GET)){
			$q1 = mysqli_query($koneksi,"select*from detail_pesanan where pesanan_id='$_GET[id]'");
			$total = 0;
			$dataPesanan = mysqli_fetch_object(mysqli_query($koneksi,"Select * from pesanan where id='$_GET[id]'"));
			$kota = $dataPesanan->kota;
			$ongkir = $dataPesanan->ongkir;
		 while($data=mysqli_fetch_object($q1)){ ?>
					<?php
						$katpro = mysqli_query($koneksi,"select*from produk where id='$data->produk_id'");
...
<?php
		if($_GET['act'] == 'bayar' && $_GET['id']){
			if(!empty($_POST)){
				...
				extract($_POST);
				$q = mysqli_query($koneksi,"insert into pembayaran Values(NULL,'$_GET[id]','$_SESSION[iam_user]','$gambar','$bayar','pending','$keterangan',NOW())");
				if($q){
			...
			extract($_GET);
			$pesanan = mysqli_fetch_object(mysqli_query($koneksi,"Select*from pesanan where id='$id'"));
			$qPembayaran = mysqli_query($koneksi,"Select * from pembayaran where id_pesanan='$id' and status='verified'") or die (mysqli_error());
			$totalPembayaran = 0;
		...
		?>
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<label>Keterangan pembayaran</label><br>
...
?>
	...
	<?php while($data=mysqli_fetch_object($q)){ ?>
			<tr <?php if($data->read == 0 ){ echo 'style="background:#cce9f8 !important;"'; } ?> >
				...
				<?php
					$katpro = mysqli_query($koneksi,"select*from user where id='$data->user_id'");
