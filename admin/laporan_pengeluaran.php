<?php
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($koneksi,"delete from laporan WHERE id_pengeluaran='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			extract($_POST);
			$q = mysqli_query($koneksi,"insert into laporan Values(NULL,'$nama_barang','$Tanggal_pengeluaran','$harga','$jumlah',$harga*$jumlah)");
			if($q){ alert("Success"); redir("input_pengeluaran.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"update laporan SET nama_barang='$nama_barang',harga='$harga',jumlah='$jumlah',total='$total' where id_pengeluaran=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("input_pengeluaran.php"); }
		}
	}
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($koneksi,"select*from laporan where id_pengeluaran='$_GET[id]'"));
				?>
					<div class="row col-md-6">
	   ...
	   ?>
		...
		<tr>
					...
					<td>
						<font size="3"><?php echo "Rp. ". number_format($semua_total, 2, ",", "."); ?></font>
					</td>
					<td></td>
				</tr>
