<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($koneksi,"delete from user WHERE id='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"insert into user Values(NULL,'$nama','$email','$telephone','$alamat','$password','$status')");
			if($q){ alert("Success"); redir("user.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			extract($_POST); 
			$password = md5($password);
			$q = mysqli_query($koneksi,"update user SET nama='$nama',email='$email',telephone='$telephone',alamat='$alamat',password='$password',status='$status' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("user.php"); }
		}
	}
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($koneksi,"select*from user where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
