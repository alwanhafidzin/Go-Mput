...
<?php	if(!empty($_GET['id'])){ ?>
		<?php
			extract($_GET); 
			$k = mysqli_query($koneksi,"SELECT * FROM produk where id='$id'"); 
		...
		<div class="col-md-9">
			...
			<h3>Detail : <?php echo $data['nama'] ?></h3>
				...
				<?php $kat = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM kategori_produk where id='$data[kategori_produk_id]'"));  ?>
					Kategori :<a href="<?php echo $url; ?>menu.php?kategori=<?php echo $kat['id'] ?>"><?php echo $kat['nama'] ?></a>
...
<?php }elseif(!empty($_GET['kategori'])){ ?>	
		...
		<?php
			extract($_GET); 
			$kat = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM kategori_produk where id='$kategori'")); 
		?>
		<div class="col-md-9">
			<div class="row">
			<div class="col-md-12">
			...
			<h3>Kategori : <?php echo $kat['nama'] ?></h3>
				<?php 
					$k = mysqli_query($koneksi,"SELECT * FROM produk where kategori_produk_id='$kategori'");
					while($data = mysqli_fetch_array($k)){
				?>
				<div class="col-md-4 content-menu">
					<a href="<?php echo $url; ?>menu.php?id=<?php echo $data['id'] ?>">
						<img src="<?php echo $url; ?>uploads/<?php echo $data['gambar'] ?>" width="230px" height="200px">
...
<?php }else{ ?>	
			...
			<h3>Daftar Semua Menu</h3>
				<?php 
					$k = mysqli_query($koneksi,"SELECT * FROM produk");
					while($data = mysqli_fetch_array($k)){
				?>
				<div class="col-md-4 content-menu">
					<a href="<?php echo $url; ?>menu.php?id=<?php echo $data['id'] ?>">
