...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($koneksi,"select*from pembayaran order by status asc");
			$j = mysqli_num_rows($q);
		?>
		...
		<?php
			if(!empty($_GET)){
				if ($_GET['act'] == "info") {
					$qInfo = mysqli_query($koneksi,"Select * from info_pembayaran limit 1") or die (mysql_error());
					$dInfo = mysqli_fetch_object($qInfo);
					if (!empty($_POST)) {
						extract($_POST);
						$query = mysqli_query($koneksi,"Update info_pembayaran set info='$info' where id='1'") or die (mysql_error());
						if ($query) {
				...
				if ($_GET['act'] == "verified") {
					extract($_GET);
					$updatePembayaran = mysqli_query($koneksi,"update pembayaran set status = 'verified' where id='$_GET[id]'");
					if ($updatePembayaran) {
						$qPembayaran = mysqli_query($koneksi,"Select * from pembayaran where id_pesanan='$id_pesanan' and status='verified'") or die (mysql_error());
						$totalPembayaran = 0;
						while ($d = mysqli_fetch_object($qPembayaran)) {
							$totalPembayaran += $d->total;
						}
						$q1 = mysqli_query($koneksi,"select*from detail_pesanan where pesanan_id='$id_pesanan'");
						$totalBayar = 0;
						while($data2=mysqli_fetch_object($q1)){
							$katpro1 = mysqli_query($koneksi,"select*from produk where id='$data2->produk_id'");
							$a = mysqli_fetch_object($katpro1);
							$totalBayar += ($a->harga * $data2->qty);
						}

						$q3 = mysqli_query($koneksi,"select*from pesanan where id='$id_pesanan'") or die (mysqli_error());
						$pesanan = mysqli_fetch_object($q3);

						$totalBayar += $pesanan->ongkir;
						if ($totalBayar <= $totalPembayaran) {
							$updatePesanan = mysqli_query($koneksi,"update pesanan set status='lunas' where id='$id_pesanan'") or die (mysqli_error());
							if ($updatePesanan) {
				...
				if ($_GET['act'] == "detail") {
					$dataPembayaran = mysqli_fetch_object(mysqli_query($koneksi,"Select * from pembayaran where id=$_GET[id]"));
					$dataPesanan = mysqli_fetch_object(mysqli_query($koneksi,"Select*from pesanan where id='$dataPembayaran->id_pesanan'"))
					?>
					<div class="col-md-6">
		...
		<?php while($data=mysqli_fetch_object($q)){ ?> 
				<tr <?php if($data->status == "pending" ){ echo 'style="background:#cce9f8 !important;"'; } ?> > 
					...
					<?php
						$katpro = mysqli_query($koneksi,"select*from user where id='$data->id_user'");
