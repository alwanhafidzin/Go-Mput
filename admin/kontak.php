<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"insert into kontak Values(NULL,'$nama','$email','$subjek','$pesan')");
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"update kontak SET nama='$nama',email='$email',subjek='$subjek',pesan='$pesan' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("kontak.php"); }
		}
	}
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($koneksi,"select*from kontak where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
