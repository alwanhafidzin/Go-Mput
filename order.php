...
?>
		...
		<?php
			if(!empty($_POST['form-order'])){
				...
				$kt = mysqli_fetch_object(mysqli_query($koneksi,"SELECT *FROM kota where id='$kota'"));
				if($difference->days >= 3){
					
					$q = mysqli_query($koneksi,"insert into pesanan Values(NULL,'$tanggal_pesan','$tanggal_digunakan','$_SESSION[iam_user]','$nama','$alamat','$kt->nama','$kt->ongkir','$telephone','0','belum lunas')");
					if($q){

						$last = mysqli_fetch_array(mysqli_query($koneksi,"SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							$product = mysqli_fetch_array(mysqli_query($koneksi,"select * from produk WHERE id='$id'"));
							if(!empty($product)){

								$ins = mysqli_query($koneksi,"insert into detail_pesanan Values(NULL,'$product[id]','$qty','$last[id]')");
