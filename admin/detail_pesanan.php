<?php 
	...
	$aa = mysqli_query($koneksi,"update pesanan SET `read`='1' where id=$_GET[id]") or die(mysqli_error());
...
?> 
	...
	<div class="container">
		<?php
			$q = mysqli_query($koneksi,"select*from pesanan where id='$_GET[id]'");
			$data = mysqli_fetch_object($q);
			$ongkir = $data->ongkir;
			$kota = $data->kota;
			$dataPembayaran = mysqli_query($koneksi,"select * from pembayaran where id_pesanan='$data->id' and status='verified'") or die (mysql_error());
			$totalPembayaran = 0;
			while ($d = mysqli_fetch_array($dataPembayaran)) {
				$totalPembayaran += $d['total'];
			}

			$q1 = mysqli_query($koneksi,"select*from detail_pesanan where pesanan_id='$data->id'");
			$totalBayar = 0;
		...
		<thead> 
			...
			<tbody> 
		 <?php 
			$q = mysqli_query($koneksi,"select*from detail_pesanan where pesanan_id='$_GET[id]'");
			$total = 0;
