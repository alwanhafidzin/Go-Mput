<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = mysqli_query($koneksi,"delete from produk WHERE id='$_GET[id]'");
			if($q){ alert("Produk berhasil di hapus"); redir("produk.php"); }
		}  
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"insert into produk Values(NULL,'$nama','$deskripsi','$gambar','$harga','$kategori_produk_id')");
			if($q){
				$upload = move_uploaded_file($_FILES['gambar']['tmp_name'], '../uploads/'.$gambar);
				if($upload){ alert("Produk berhasil ditambah"); redir("produk.php"); }
			}
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){ 
			...
			$q = mysqli_query($koneksi,"update produk SET nama='$nama',deskripsi='$deskripsi', gambar='$gambar', harga='$harga', kategori_produk_id='$kategori_produk_id' where id=$_GET[id]") or die(mysql_error());
			if($q){
				$upload = move_uploaded_file($_FILES['gambar']['tmp_name'], '../uploads/'.$gambar);
				if($upload){ alert("Edit Produk Berhasil"); redir("produk.php"); }
			}
		}
	}
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				?>
					...
					<form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kategori_produk_id" required class="form-control"> 
							<?php
								$katpro = mysqli_query($koneksi,"select*from kategori_produk");
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($koneksi,"select*from produk where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
					<form action="produk.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<select name="kategori_produk_id" required class="form-control"> 
						<?php
								$katpro = mysqli_query($koneksi,"select*from kategori_produk where id='$data->kategori_produk_id'");
