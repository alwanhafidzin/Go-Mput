<?php 
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = mysqli_query($koneksi,"update pesanan SET nama='$nama',tanggal_pesan='$tanggal_pesan',tanggal_digunakan='$tanggal_digunakan',user_id='$user_id',alamat='$alamat',telephone='$telephone' where id=$_GET[id]") or die(mysql_error());
			if($q){ alert("Success"); redir("pesanan.php"); }
		}
	}
...
?> 
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){ 
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object(mysqli_query($koneksi,"select*from pesanan where id='$_GET[id]'"));
				?>
					...
					<form action="pesanan.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<?php
							...
							<?php
								$katpro = mysqli_query($koneksi,"select*from user");
